"""TurboQuant-inspired compressor: rotation + scalar quantization.

This module implements a compression approach INSPIRED BY the TurboQuant paper's
core ideas, but does NOT claim to exactly reproduce the paper. The key insight
is that applying a random orthogonal rotation (or Hadamard-style transform)
before quantization spreads outlier magnitudes more evenly across dimensions,
...
The rotation matrix is fixed and deterministic (seeded), so it does not need
to be stored per tensor — only the seed must be consistent.

Design notes:
- The rotation is applied along the last dimension (head_dim for KV, hidden_dim
  for hidden states).
- For the Hadamard variant, we use a fast Walsh-Hadamard transform approximation
  rather than storing the full matrix (O(d log d) instead of O(d^2)).
- Residual correction (quantizing the error from the first pass) is an optional
  extension flagged in config but not fully implemented in this first version.
"""
...
logger = logging.getLogger("msa_turboquant.compression.turboquant_like")
...
class TurboQuantLikeCompressor(BaseCompressor):
    """TurboQuant-inspired compressor: rotation + groupwise scalar quantization.

    This is clearly labeled as INSPIRED BY TurboQuant, not an exact reproduction.
    ...
    def __init__(
        ...
        # Cache rotation matrices per dimension to avoid recomputing
    ...
    def _get_rotation_matrix(self, dim: int, device: torch.device) -> torch.Tensor:
        ...
        if dim not in self._rotation_cache:
            if self._rotation == "random_orthogonal":
                ...
                # For Hadamard, we don't store a matrix — handled separately
                # But we still cache the identity-sized placeholder
    ...
    def _inverse_rotate(self, tensor: torch.Tensor, original_dim: int) -> torch.Tensor:
        ...
        if self._rotation == "hadamard":
            # WHT is its own inverse (involutory, already normalized)
    ...
    @property
    def name(self) -> str:
        return f"turboquant_like_{self._rotation}_{self._bits}b_g{self._group_size}"
    ...
    def compute_dot_product_error(
        ...
        # Compute attention-like dot products
        # queries: (..., q_len, dim), keys: (..., k_len, dim)
        q = queries.float()
        k_orig = keys.float()
        k_recon = keys_hat.float()

        scores_orig = q @ k_orig.transpose(-2, -1)  # (..., q_len, k_len)
